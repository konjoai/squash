...
">
  ...
  <div style="display:flex;gap:12px;margin-top:20px;flex-wrap:wrap;">
    <div style="background:#ffffff0a;border:1px solid #ffffff12;border-radius:8px;padding:10px 18px;text-align:center;min-width:90px;">
...
def _meta_card(m: dict) -> str:
    ...
    rows = "".join(
        f"""<tr>
          <td style="padding:8px 12px;color:#64748b;font-size:0.82rem;white-space:nowrap;
                     font-weight:500;text-transform:uppercase;letter-spacing:0.05em;width:35%;">{k}</td>
          <td style="padding:8px 12px;font-size:0.88rem;">{v}</td>
        </tr>"""
        for k, v in fields
    )
    return f"""
...
def _findings_block(findings: list, license_warnings: list) -> str:
    ...
    if findings:
        ...
        for f in findings[:25]:
            ...
            rows += f"""<tr style="border-top:1px solid #ffffff08;">
              <td style="padding:10px 14px;white-space:nowrap;">{badge}</td>
              <td style="padding:10px 14px;font-family:monospace;font-size:0.8rem;color:#94a3b8;">{f.get("finding_id","")}</td>
              <td style="padding:10px 14px;font-size:0.87rem;color:#e2e8f0;">{f.get("title","")}</td>
              <td style="padding:10px 14px;font-size:0.78rem;color:#475569;font-family:monospace;">{f.get("file_path","")}</td>
            </tr>"""
        if len(findings) > 25:
...
<div style="background:#ffffff05;border:1px solid #ef444420;border-radius:12px;padding:20px;margin-top:14px;">
  ...
  <div style="overflow-x:auto;margin:-4px;">
...
def run_scan(model_id: str, hf_token: str, download_weights: bool) -> tuple:
...
<div style="background:#ef444412;border:1px solid #ef444440;border-radius:12px;padding:20px;
  ...
  <span style="font-size:1.4rem;">❌</span>
  <div>
    <div style="color:#ef4444;font-weight:700;margin-bottom:4px;">Scan Failed</div>
    <div style="color:#fca5a5;font-size:0.88rem;font-family:monospace;">{exc}</div>
  </div>
</div>"""
...
/* Inputs */
input, textarea {
  background: var(--sq-surface2) !important;
  color: var(--sq-text) !important;
  border-color: var(--sq-border) !important;
...
#scan-btn > button:active {
  transform: translateY(0) !important;
}

/* Checkbox */
input[type="checkbox"] {
  accent-color: var(--sq-accent) !important;
}
...
.examples tbody tr:hover td { background: var(--sq-surface2) !important; color: var(--sq-text) !important; cursor: pointer; }
...
with gr.Blocks(title="Squash Scanner — HuggingFace Model Security", css=CSS) as demo:
    ...
    <div style="text-align:center;padding:36px 0 24px;">
      ...
      <div style="display:flex;justify-content:center;gap:8px;margin-top:18px;flex-wrap:wrap;">
        <span style="background:#6366f118;border:1px solid #6366f130;color:#818cf8;
                     padding:4px 12px;border-radius:20px;font-size:0.75rem;font-weight:600;">🐍 Pickle / PyTorch</span>
        <span style="background:#f59e0b12;border:1px solid #f59e0b25;color:#fbbf24;
                     padding:4px 12px;border-radius:20px;font-size:0.75rem;font-weight:600;">⚠️ GGUF injection</span>
        <span style="background:#10b98112;border:1px solid #10b98125;color:#34d399;
                     padding:4px 12px;border-radius:20px;font-size:0.75rem;font-weight:600;">🔏 safetensors</span>
        <span style="background:#3b82f612;border:1px solid #3b82f625;color:#60a5fa;
                     padding:4px 12px;border-radius:20px;font-size:0.75rem;font-weight:600;">📜 License risk</span>
        <span style="background:#8b5cf618;border:1px solid #8b5cf630;color:#a78bfa;
                     padding:4px 12px;border-radius:20px;font-size:0.75rem;font-weight:600;">🏛️ EU AI Act</span>
      </div>
    </div>
    """)
